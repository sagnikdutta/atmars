<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
<title>AtMars Info</title>
<link rel="stylesheet" type="text/css" href="css/info.css">
</head>

<body>
<div class="cloud"></div>
<div class="container">
  <div class="filter"></div>
  <div class="header"> <img src="register-img/register.png" width="917" class="register"/>
    <!-- end .header --></div>
    
  <div style="top:0; height:117px; background-color:#A1E0E9"></div>
  <div class="content">
  	<div style="margin-top:100px; margin-left:50px; margin-right:50px; margin-bottom:80px;">
    <div style="float:left"><img src="info-img/info.png" /></div>
    <div style="float:left; line-height:80px; font-family:'Comic Sans MS', cursive; font-size:36px">&nbsp;&nbsp;&nbsp;Info</div>
    <div style="float:left; margin-left:80px; margin-right:80px; font-family:'Comic Sans MS', cursive;">
    <p>&nbsp;&nbsp;&nbsp;&nbsp;Sorry. Your operation cannot be accepted by our server. We will record this error information and correct it soon.</p>
    </div>
    <div style="clear:left"></div>
    </div>

  </div>
  <div class="footer">
    <p>&nbsp;</p>
  <!-- end .footer --></div>
<!-- end .container --></div>
</body>
</html>

